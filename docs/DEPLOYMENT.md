...
Email HTML is composed in-repo (`apps/server/src/services/email-templates.ts`). Links are plain-text URLs (no mismatched `<a>` labels) and click tracking is disabled to satisfy SendByte’s abuse filter.
